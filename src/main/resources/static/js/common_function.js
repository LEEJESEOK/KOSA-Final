...
var layerAlert = function (msg, refresh, fixedYn) {
    ...
    alertTag += '</div>';
    alertTag += '</div>';
    alertTag += '<a href="javascript:void(0);" class="btn_close"><img src="http://www.thehandsome.com/_ui/desktop/common/images/popup/ico_close.png" alt="닫기"></a>';
    alertTag += '</div>';
    alertTag += '</div>';
...
var closeNotConfirmLayerAlert = function (msg, refresh, fixedYn, isFreeGift) {
    ...
    alertTag += '</div>';
    alertTag += '</div>';
    alertTag += '<a href="javascript:void(0);" class="btn_close"><img src="https://www.thehandsome.com/_ui/desktop/common/images/popup/ico_close.png" alt="닫기"></a>';
    alertTag += '</div>';
    alertTag += '</div>';
...
var ValidationCheck = function (formId) {
    ...
    this.isValid = function () {
        ...
        $.each(this.checkIdList, function () {
            ...
            console.log('파라미터로 2~ : ' + $this.length);
...
var searchAddrLayer = function (addr1, addr2, zipcode) {
    ...
    addressLayerTag += '    </div>                                                                                                ';
    addressLayerTag += '    <!--//contents-->                                                                                     ';
    addressLayerTag += '    <a href="javascript:void(0);" class="btn_close"><img src="https://www.thehandsome.com/_ui/desktop/common/images/popup/ico_close.png" alt="닫기"></a>';
    addressLayerTag += '</div>';
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $.each(response, function () {
                        ...
                        var ssln = '';
                        if (this.startSubLotNumber != '0') {
                            ssln = '-' + this.startSubLotNumber;
                        }
                        row += ssln;
                        var eln = '';
...
var searchAddressLayer = function (addr1, addr2, zipcode) {
    ...
    addressLayerTag += '        </div>                                                                                            ';
    addressLayerTag += '    </div>                                                                                                ';
    addressLayerTag += '    <a href="#;" class="btn_close"><img src="https://www.thehandsome.com/_ui/desktop/common/images/popup/ico_close.png" alt="닫기"></a>';
    addressLayerTag += '</div>';
...
function getDecimalToString(val, intIdx, decIdx, option) {
    ...
    function _optionCalc(_value, _calcOption) {
        ...
        switch (_calcOption) {
            case "ceil" :
                return String(Math.ceil(intValue));
                break;
            case "floor" :
                return String(Math.floor(intValue));
                break;
            default :
